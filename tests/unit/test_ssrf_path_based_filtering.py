...
class TestPathBasedFilteringBasics:
    ...
    def test_blocked_path_on_allowed_domain(self):
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/admin/users"}
        )
    ...
    def test_combined_allowed_and_blocked_paths(self):
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://api.corp.local/public/data"}
        )
        assert not should_block, "Should allow /public/data"

        # Blocked subset of allowed path - should block
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://api.corp.local/public/admin/settings"}
        )
        assert (
            should_block
        ), "Should block /public/admin/* even though /public/* is allowed"

        # Exact allowed path - should allow
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://api.corp.local/health"}
        )
...
class TestGlobPatterns:
    ...
    def test_single_wildcard_matches_single_level(self):
        ...
        protector = SSRFProtector(
            {
                "path_based_rules": [
                    {"domain": "example.com", "blocked_paths": ["/api/*/delete"]}
                ]
            }
        )
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/api/users/delete"}
        )
        assert should_block, "Should block /api/users/delete (matches /api/*/delete)"

        # Should NOT match multiple levels
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/api/v1/users/delete"}
        )
        assert (
            not should_block
        ), "Should NOT block /api/v1/users/delete (too many levels for single *)"
    ...
    def test_double_wildcard_matches_recursive(self):
        ...
        protector = SSRFProtector(
            {
                "path_based_rules": [
                    {"domain": "example.com", "blocked_paths": ["/admin/**"]}
                ]
            }
        )
    ...
    def test_question_mark_matches_single_char(self):
        ...
        protector = SSRFProtector(
            {
                "path_based_rules": [
                    {"domain": "example.com", "allowed_paths": ["/v?/api/*"]}
                ]
            }
        )
...
class TestQueryParameters:
    ...
    def test_query_params_included_in_match(self):
        ...
        protector = SSRFProtector(
            {
                "path_based_rules": [
                    {"domain": "example.com", "blocked_paths": ["/api/users?admin=*"]}
                ]
            }
        )
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/api/users?admin=true"}
        )
    ...
    def test_wildcard_matches_query_params(self):
                ...
                "path_based_rules": [
                    {
                        ...
                        "blocked_paths": [
                            "/debug*"
                        ],  # Matches /debug, /debug?foo=bar, etc
                    }
                ]
            }
        )
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/debug?verbose=true"}
        )
...
class TestTrailingSlashes:
    ...
    def test_trailing_slash_normalization(self):
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/admin/"}
        )
        ...
        protector = SSRFProtector(
            {
                ...
                "path_based_rules": [
                    {"domain": "example.com", "allowed_paths": ["/"]}  # Allow only root
                ],
            }
        )
...
class TestCaseSensitivity:
    ...
    def test_domain_case_insensitive(self):
        ...
        protector = SSRFProtector(
            {
                "additional_blocked_domains": ["Internal.API.com"],
                "path_based_rules": [
                    {"domain": "Internal.API.com", "allowed_paths": ["/public/*"]}
                ],
            }
        )
        ...
        for domain in ["internal.api.com", "INTERNAL.API.COM", "Internal.Api.Com"]:
            should_block, msg = protector.check(
                "Bash", {"command": f"curl http://{domain}/public/data"}
            )
            assert not should_block, f"Should allow /public/data on {domain}"

            should_block, msg = protector.check(
                "Bash", {"command": f"curl http://{domain}/admin"}
            )
...
class TestEdgeCases:
    ...
    def test_no_path_rules_for_domain(self):
        ...
        protector = SSRFProtector(
            {
                ...
                "path_based_rules": [
                    {"domain": "other.com", "allowed_paths": ["/public/*"]}
                ],
            }
        )
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://blocked.com/anything"}
        )
    ...
    def test_empty_path_lists(self):
        ...
        protector = SSRFProtector(
            {
                "additional_blocked_domains": ["example.com"],
                "path_based_rules": [
                    {"domain": "example.com", "allowed_paths": [], "blocked_paths": []}
                ],
            }
        )
        ...
        should_block, msg = protector.check(
            "Bash", {"command": "curl http://example.com/anything"}
        )
        ...
        protector = SSRFProtector(
            {
                ...
                "path_based_rules": [
                    {"domain": "api1.internal", "allowed_paths": ["/public/*"]},
                    {
    ...
    def test_immutable_protections_not_affected_by_path_rules(self):
        ...
        assert (
            should_block
        ), "Metadata endpoints are immutable, path rules should not override"
