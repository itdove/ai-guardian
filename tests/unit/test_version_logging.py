...
class VersionLoggingTest(TestCase):
    ...
    def test_record_factory_adds_version(self):
        ...
        self.assertTrue(
            hasattr(record, "version"), "Log record should have version attribute"
        )
        self.assertEqual(
            record.version,
            ai_guardian.__version__,
    ...
    def test_version_format_in_file_handler(self):
        ...
        self.assertTrue(
            hasattr(ai_guardian, "_file_handler"),
    ...
    def test_startup_version_logging(self):
        ...
        if log_file.exists():
            with open(log_file, "r", encoding="utf-8") as f:
                ...
                lines = f.readlines()
                recent_lines = (
                    "".join(lines[-100:]) if len(lines) > 100 else "".join(lines)
                )
    ...
    def test_formatter_structure(self):
        ...
        self.assertIn(
            "v%(version)s", fmt, "Format should include version with 'v' prefix"
        )
        ...
        self.assertEqual(
            datefmt, "%Y-%m-%d %H:%M:%S", "Date format should be YYYY-MM-DD HH:MM:SS"
        )
