...
class TestReadKiroTranscript(unittest.TestCase):
    ...
    def test_read_all_lines(self):
        ...
        text, new_pos = _read_jsonl_incremental(
            path, 0, _extract_text_from_kiro_entry, label="Kiro"
        )
    ...
    def test_incremental_read_skips_seen(self):
        ...
        first_line_bytes = len(json.dumps(entries[0]).encode("utf-8")) + 1
        text, new_pos = _read_jsonl_incremental(
            path, first_line_bytes, _extract_text_from_kiro_entry, label="Kiro"
        )
    ...
    def test_file_not_found(self):
        text, new_pos = _read_jsonl_incremental(
            "/nonexistent/session.jsonl", 0, _extract_text_from_kiro_entry, label="Kiro"
        )
        self.assertEqual(text, "")
        self.assertEqual(new_pos, 0)
    ...
    def test_truncated_file_rereads_from_start(self):
        ...
        text, new_pos = _read_jsonl_incremental(
            path, 99999, _extract_text_from_kiro_entry, label="Kiro"
        )
    ...
    def test_nothing_new(self):
        ...
        text, new_pos = _read_jsonl_incremental(
            path, file_size, _extract_text_from_kiro_entry, label="Kiro"
        )
        self.assertEqual(text, "")
