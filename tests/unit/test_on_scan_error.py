...
DISABLE_ALL_SCANNERS = {
    'ai_guardian.hook_processing._load_secret_scanning_config': ({"enabled": False}, None),
    ...
    'ai_guardian.hook_processing._load_prompt_injection_config': ({"enabled": False}, None),
    'ai_guardian.hook_processing._load_config_scanner_config': ({"enabled": False}, None),
    'ai_guardian.hook_processing._load_permissions_config': ({"enabled": False}, None),
    'ai_guardian.hook_processing._load_transcript_scanning_config': ({"enabled": False}, None),
}
...
class TestOnScanErrorToolPolicy(TestCase):
    ...
    def test_tool_policy_error_allow(self, mock_pattern, mock_secret, mock_perms, mock_on_error, mock_pii, mock_extract):
        ...
        with patch('ai_guardian.hook_processing.ToolPolicyChecker', side_effect=Exception("Policy check crashed")):
            hook_data = {
...
class TestOnScanErrorPromptInjection(TestCase):
    ...
    def test_prompt_injection_error_allow(self, mock_pattern, mock_secret, mock_perms, mock_injection, mock_on_error, mock_pii, mock_extract):
        ...
        with patch('ai_guardian.hook_processing.HAS_PROMPT_INJECTION', True), \
             patch('ai_guardian.hook_processing.PromptInjectionDetector', side_effect=Exception("Injection check crashed")):
            hook_data = {
                "hook_event_name": "PreToolUse",
                "tool_name": "Read",
                "tool_use": {"name": "Read", "parameters": {"file_path": "/tmp/test.txt"}},
...
class TestOnScanErrorSecretScanning(TestCase):
    ...
    def test_scanner_unavailable_allow(self, mock_pattern, mock_secret, mock_on_error):
        ...
        with patch('ai_guardian.hook_processing.HAS_SCANNER_ENGINE', True), \
             patch('ai_guardian.hook_processing.select_engine', side_effect=RuntimeError("No scanner")):
...
class TestOnScanErrorTranscriptScanning(TestCase):
    ...
    def test_transcript_scan_error_allow(self, mock_pattern, mock_secret, mock_transcript, mock_on_error, mock_pii):
        ...
        with patch('ai_guardian.hook_processing._get_transcript_path', return_value="/tmp/transcript.jsonl"), \
             patch('ai_guardian.hook_processing.scan_transcript_incremental', side_effect=Exception("Transcript scan crashed")):
            hook_data = {
...
class TestOnScanErrorPiiScanning(TestCase):
    ...
    def test_scan_for_pii_error_allow(self):
        ...
        with patch('ai_guardian.hook_processing._get_on_scan_error_action', return_value='allow'), \
    ...
    def test_scan_for_pii_error_block(self):
        ...
        with patch('ai_guardian.hook_processing._get_on_scan_error_action', return_value='block'), \
             patch('ai_guardian.secret_redactor.SecretRedactor', side_effect=Exception("PII scan crashed")):
            has_pii, text, redactions, warning = ai_guardian._scan_for_pii("test text", pii_config)
