...
def create_offensive_language_page(service, daemon_name: str):
    ...
    sidebar = create_sidebar(daemon_name, current=f"/{daemon_name}/offensive-language")
    ...
    with ui.column().classes("flex-grow p-6 gap-4"):
        ...
        with ui.card().classes("w-full"):
            with ui.row().classes("items-center gap-1 mb-2"):
                ...
                field_help_icon("scan_offensive")
        ...
        def on_save():
            ...
            ign = [
                ln.strip() for ln in ignore_files_area.value.splitlines() if ln.strip()
            ]
            existing = _load()
            existing.update(
                {
