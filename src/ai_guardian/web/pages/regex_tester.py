...
def _test_regex(
    pattern, text, case_insensitive=True, multiline=True, max_matches=MAX_MATCHES
):
...
def create_regex_tester_page(service, daemon_name: str):
    ...
    sidebar = create_sidebar(daemon_name, current=f"/{daemon_name}/regex-tester")
    create_header(daemon_name, drawer=sidebar)

    with ui.column().classes("flex-grow p-6 gap-4"):
        ui.label("Regex Tester").classes("text-2xl font-bold")
        ui.label("Test regex patterns and add them to config allowlists.").classes(
            "text-xs text-grey-6"
        )
        ...
        with ui.card().classes("w-full"):
            ...
            def on_pattern_change(e):
                ...
                if safe:
                    ...
                    validation_label.classes(replace="text-xs text-green")
                else:
                    validation_label.text = msg
                    validation_label.classes(replace="text-xs text-amber")

            pattern_input.on("update:model-value", on_pattern_change)

        with ui.card().classes("w-full"):
            ui.label("Test Text").classes("text-lg font-bold")
            text_input = (
                ui.textarea(placeholder="Enter sample text to test against")
                .props("outlined")
                .classes("w-full")
        ...
        with ui.row().classes("items-center gap-4"):
            ci_check = ui.checkbox("Case Insensitive", value=True)
        ...
        async def do_test():
            ...
            matches, err = _test_regex(pat, txt, ci_check.value, ml_check.value)

            results_container.clear()
            with results_container:
                with ui.card().classes("w-full"):
                    if err:
                        ui.label(f"Error: {err}").classes("text-sm text-red")
                    else:
                        count = len(matches)
                        suffix = (
                            f" (showing {MAX_MATCHES})" if count >= MAX_MATCHES else ""
                        )
                        ui.label(f"Matches: {count}{suffix}").classes(
                            "text-lg font-bold"
                        )
                            ...
                            with (
                                ...
                                for m in matches:
                                    with ui.row().classes("items-center gap-2"):
                                        ...
                                        ui.label(f"[{m['start']}:{m['end']}]").classes(
                                            "text-xs text-grey-6"
                                        )
                                        ui.label(repr(m["match"])).classes(
                                            "text-sm"
                                        ).style("font-family: monospace")

        ui.button("Test", icon="play_arrow", on_click=do_test).props("dense")

        with ui.card().classes("w-full"):
            ui.label("Add to Config").classes("text-lg font-bold")
            ui.label("Add the current pattern to an allowlist in config.").classes(
                "text-xs text-grey-6"
            )
            ...
            async def add_pattern():
                ...
                ui.notify(f"Added to {section}", type="positive")

            ui.button("Add Pattern", icon="add", on_click=add_pattern).props("dense")
