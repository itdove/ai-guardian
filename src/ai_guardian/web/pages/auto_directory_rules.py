            ...
            with content:
                ...
                with ui.card().classes("w-full"):
                    ...
                    with ui.row().classes("items-center gap-2 w-full"):
                        ...
                        async def on_toggle_symlinks(e):
                            ...
                            await run.io_bound(save_web_config, cfg)
                            ui.notify(
                                "Symlinks " f"{'allowed' if e.value else 'disallowed'}",
                                type="positive",
                            )
                ...
                if discovery["error"]:
                    with ui.card().classes("w-full"):
                        ...
                        ui.label(discovery["error"]).classes("text-xs text-grey-6 ml-8")
                ...
                with ui.card().classes("w-full"):
                    ...
                    with ui.row().classes("gap-6 ml-8 mt-2"):
                        ...
                        with ui.column().classes("gap-0"):
                            ui.label(str(n_discovered)).classes("text-2xl font-bold")
                            ui.label("Skills discovered").classes("text-xs text-grey-6")
                        with ui.column().classes("gap-0"):
                            ui.label(str(n_matched)).classes("text-2xl font-bold")
                            ui.label("Skills matched").classes("text-xs text-grey-6")
                        with ui.column().classes("gap-0"):
                            ui.label(str(n_rules)).classes("text-2xl font-bold")
                            ui.label("Rules generated").classes("text-xs text-grey-6")

                # --- Skill patterns card ---
                with ui.card().classes("w-full"):
                    ui.label("Skill Permission Patterns").classes("text-sm font-bold")
                    ui.label(
                        ...
                        for name in sorted(discovery["discovered_skills"].keys()):
                            ...
                            with ui.row().classes("items-center gap-2"):
                                ...
                                ui.label(name).classes(
                                    "text-sm font-bold"
                                    + (" text-green" if is_match else " text-grey-6")
                                )
                                for p in paths:
                                    ui.label(p).classes("text-xs text-grey-6 font-mono")
                    else:
                        ui.label("No skills discovered in skill directories.").classes(
                            "text-grey-6 text-sm"
                        )
                ...
                with ui.card().classes("w-full"):
                    ...
                    if discovery["generated_rules"]:
                        for idx, rule in enumerate(discovery["generated_rules"]):
                            with (
                                ...
                                with ui.row().classes("items-center gap-2"):
                                    ...
                                    if source:
                                        ui.label(source).classes("text-xs text-grey-6")
                        ...
                        if not is_enabled:
                            ui.label(
                                "Enable auto directory rules to see " "generated rules."
                            ).classes("text-grey-6 text-sm")
