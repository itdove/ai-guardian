...
def create_permission_rules_page(service, daemon_name: str):
    ...
    sidebar = create_sidebar(daemon_name, current=f"/{daemon_name}/permission-rules")
    ...
    with ui.column().classes("flex-grow p-6 gap-4"):
        ...
        ui.label(
            "All tool permission rules in evaluation order. " "Last matching rule wins."
        ...
        async def refresh():
            ...
            with content:
                        ...
                        for idx, rule in enumerate(all_rules):
                            ...
                            with (
                                ...
                                with ui.row().classes("items-center gap-2 w-full"):
                                    ...
                                    ui.badge(t_label, color=t_color).classes("text-xs")
                                    ui.badge(mode.upper(), color=mode_color).classes(
                                        "text-xs"
                                    )
                                    ...
                                    for pi, p in visible_pats:
                                        ...
                                        with (
                                            ...
                                            if not immutable:
                                                ...
                                                # Delete pattern
                                                async def do_del_pat(ri=idx, pii=pi):
                                                    cfg = await run.io_bound(
                                                        load_web_config
                                                    )
                                                    pr = cfg.get(
                ...
                # --- Info card ---
                with ui.card().classes("w-full"):
                    ui.label("How Rule Evaluation Works").classes("text-sm font-bold")
                    ui.label(
