...
def create_cache_status_page(service, daemon_name: str):
    ...
    sidebar = create_sidebar(daemon_name, current=f"/{daemon_name}/cache-status")
            ...
            with summary_row:
                ...
                with ui.card().classes("p-3"):
                    ...
                    if last_reload:
                        ...
                        ui.label(dt.strftime("%H:%M:%S")).classes("text-2xl font-bold")
                    else:
            ...
            with content:
                ...
                for proj in projects:
                    ...
                    with ui.card().classes("w-full"):
                        ...
                        ui.label(project_dir).classes("text-xs text-grey-6 font-mono")

                        ui.separator()

                        with ui.grid(columns=2).classes("w-full gap-x-8 gap-y-2"):
                            ui.label("Project Config").classes("text-xs text-grey-6")
                            ui.label(_short_path(proj.get("config_path"))).classes(
                                "text-sm font-mono"
                            )

                            ui.label("Config Modified").classes("text-xs text-grey-6")
                            ui.label(_format_mtime(proj.get("config_mtime"))).classes(
                                "text-sm"
                            )

                            ui.label("Last Seen").classes("text-xs text-grey-6")
                            ui.label(
                                _format_seconds_ago(proj.get("last_seen_seconds_ago"))
                            ).classes("text-sm")

                            gp = proj.get("global_config_path")
                            if gp:
                                ui.label("Global Config").classes("text-xs text-grey-6")
                                ui.label(_short_path(gp)).classes("text-sm font-mono")
