...
def handle_post_tool_use(ctx=None, **kwargs):
    ...
    if has_secrets:
        ...
        if not file_path and pretool_ctx:
            file_path = pretool_ctx.get("file_path")
        ask_result = _handle_ask_mode_auto(
            ...
            findings=(post_secret_result.findings if post_secret_result else None),
        )
        ...
        if ask_result is not None:
            ...
            if ask_result.decision not in (
                ...
                _log_ask_decision(
                    ...
                    file_path=file_path,
                    line_number=(
                        post_secret_result.line_number if post_secret_result else None
                    ),
